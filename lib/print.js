...
export default async function (m, conn = { user: {} }) {
  try {
    ...
    // ⭐ QUESTA È LA PARTE CHIAVE: decode del LID per il sender
    let resolvedSender = sender
    if (/@lid/.test(sender)) {
      if (m.key?.senderPn) {
        resolvedSender = m.key.senderPn
      } else {
        // Prova a ottenere il PN dal LID usando conn.getPNForLID
        try {
    ...
    // Usa resolvedSender invece di sender per il display
